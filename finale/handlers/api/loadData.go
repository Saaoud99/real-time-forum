...
func CheckLogin(w http.ResponseWriter, r *http.Request) {
	...
	// if r.URL.Path != "/" {
	// 	utils.ResponseJSON(w, utils.Resp{Msg: "path not correct ", Code: http.StatusMethodNotAllowed})
	// 	return
	// }
	// Ensure correct method and endpoint
	// if r.Method != http.MethodGet {
	// 	utils.ResponseJSON(w, utils.Resp{Msg: "Method Not Allowed", Code: http.StatusMethodNotAllowed})
	// 	return
	// }
	...
	if err == nil {
		...
		err := database.DataBase.QueryRow("SELECT EXISTS(SELECT 1 FROM sessions WHERE session_id=?)", cookie.Value).Scan(&isValid)
		if err != nil && err != sql.ErrNoRows {
			utils.ResponseJSON(w, utils.Resp{Msg: "Error checking session", Code: http.StatusInternalServerError})
			return
		}

		// If session is valid, user is logged in
		if isValid {
			isLoggedIn = true
		} else {
	...
	// Send JSON response
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(LoginResponse{IsLoggedIn: isLoggedIn})
}
